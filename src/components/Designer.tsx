...
const Designer = () => {
  ...
  useDndMonitor({
    onDragEnd: (event: DragEndEvent) => {
      ...
      const DroppingOverElement =
        isDroppingOverBottomHalf || isDroppingOverTopHalf;

      const droppingSidebarBtnElement =
        DroppingOverElement && isDesignerBtnElement;
      ...
      const isDroppingDesignerElementOverTopHalf =
        over.data.current?.isTopHalfDesignerElement;
      const isDroppingDesignerElementOverBottomHalk =
        over.data.current?.isBottomHalfDesignerElement;

      if (
        isDroppingDesignerElementOverBottomHalk ||
        isDroppingDesignerElementOverTopHalf
      ) {
        console.log("asd");
        const type = active.data.current?.type;
        const newElement = FormElements[type as ElementsType].construct(
        ...
        if (isDroppingDesignerElementOverBottomHalk)
          indexForNewElement = overElementIndex + 1;
        removeElement(active.data.current?.elementId);
        addElement(indexForNewElement, newElement);
        return;
