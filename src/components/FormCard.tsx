...
const FormCard = ({formData}:{formData:Form}) => {
  ...
  return (
    ...
    <div className="title flex justify-between">
        ...
        {formData.published ? (
            <span className='bg-green-600 px-2 py-1 text-white font-medium rounded-md'>Published</span>
        ) : (
    ...
    <div className={`flex ${formData.published ? "justify-center w-full" : "justify-between"} items-center mt-auto`}>
        {formData.published && (
            <Link href={`/formdetails/${formData.id}`} type='button' className={`${formData.published ? "w-full" : ""} flex rounded-md gap-2 bg-indigo-500 hover:bg-indigo-600 p-2 text-white font-medium text-lg justify-center items-center`}>
                View Submission
            </Link>
        )}
        
        {!formData.published && (
            <Link href={`/form/${formData.id}`} type='button' className='flex w-full rounded-md gap-2 bg-indigo-500 hover:bg-indigo-600 p-2 text-black font-medium text-lg justify-center items-center'>
                Edit Form
            </Link>
        )}
    </div>
